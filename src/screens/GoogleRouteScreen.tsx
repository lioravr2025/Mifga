...
export default function GoogleRouteScreen({ position, ride, active }: { position: LatLng; ride: RideMonitor; active: boolean }) {
  ...
  useEffect(() => {
    // `active` matters because this screen stays mounted (just hidden) while
    // another tab is open - without it, a route planned earlier and then
    // abandoned (switched to the main tab and started a plain, destination-
    // less ride from there) kept silently auto-rerouting/chiming toward that
    // stale destination in the background.
    if (!active || !ride.rideActive || !route || !destPoint || !routesLib || reroutingRef.current) return;
